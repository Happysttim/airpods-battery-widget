...
>(({ count, className, ...props }, ref) => {
  ...
  return (
    <span
      ref={ref}
      className={cn(
        'flex items-center gap-1 text-xs text-(--status-away)',
        className,
      )}
      {...props}
    >
...
const RepositoryCardMeta = forwardRef<HTMLDivElement, RepositoryCardMetaProps>(
  ({ children, className, ...props }, ref) => (
    <div
      ref={ref}
      className={cn('px-3 pb-3 md:px-3.5 md:pb-3.5 -mt-1', className)}
      {...props}
    >
      {children}
    </div>
...
>(({ children, className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn('text-xs mt-0.5 text-(--text-secondary)', className)}
    {...props}
  >
    {children}
  </div>
...
RepositoryCardExpandedContent.displayName =
...
RepositoryCardContributionBadge.displayName =
  'RepositoryCardGlass.ContributionBadge';
...
const RepositoryCardTeamAvatars = forwardRef<
  HTMLDivElement,
  RepositoryCardTeamAvatarsProps
        ...
        <HoverCardGlass key={member.id} openDelay={200}>
          <HoverCardGlassTrigger asChild>
            <span className="inline-flex">
              <AvatarGlass
                size="sm"
                className="border-2 border-(--card-bg) cursor-pointer"
              >
                {member.avatar ? (
...
const RepositoryCardHealthStatus = forwardRef<
  HTMLDivElement,
  RepositoryCardHealthStatusProps
      ...
      <div>
        <div className="flex justify-between text-xs mb-1">
          ...
          <span style={{ color: 'var(--text-primary)' }}>
            {formatNumberWithSuffix(userCommits)} /{' '}
            {formatNumberWithSuffix(totalCommits)} ({commitPercent}%)
          </span>
        </div>
...
RepositoryCardContributionProgress.displayName =
...
const RepositoryCardGlassLegacy = forwardRef<
  HTMLDivElement,
  RepositoryCardGlassLegacyProps
>(
  (
    {
      name,
